...
> INLA is a fast method to do Bayesian inference with latent Gaussian models and `inlabru` is an `R`-package that implements this method with a flexible and simple interface and some additional functionality.
...
We define the `linear predictor` as the mean (or a function of the mean) of our observations `given` the model components.
...
One example with time series: Rainfall over 1 mm in the Tokyo area for each calendar day during two years (1983-84); binary response
...
-   $f(\text{time}_t)$ is a RW2 model (this is just a smoother; a smooth function of time). The smoothness is controlled by a hyperparameter $\tau_f$
...
We have larynx cancer mortality counts for males in 544 districts of Germany from 1986 to 1990 and want to model these in space.
...
-   Many of the models you have used (plus some you have never used but will learn about) are *just* special cases of the large class of Latent Gaussian models.
