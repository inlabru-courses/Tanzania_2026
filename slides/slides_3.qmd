...
Data are often observed in time, and observations at taken at similar points in time are often likely to be dependent.
...
**Note:** We can use the same approach to smooth covariate effects!
...
**Goal** we want to understand the pattern and predict into the future.
...
-   Random Walk of order 1 (RW1): we consider the two nearest neighbours

-   Random Walk of order 2 (RW2): we consider the four nearest neighbours
...
| Basically, it’s easy to do the theory. | Not suitable for large problems. |

. . .

We need to reduce the computational burden !

This can be done by using **sparse** matrices, i.e.\ matrices where a  lot of entries are exactly zero!!
...
-   We will see how the idea of GMRF expands also to continuously indexed processes (SPDE approach).
...
A common option are the so-called *PC-priors*
...
-   RW2 yields smoother results than RW1.
...
-   The function `inla.group()` bins covariate values into groups (default 25 groups).
...
::::: columns
::: {.column width="50%"}
The data are derived from an study of 892 females under 50 years in three Gambian villages in West Africa.
...
-   RW1 models can be seen as a limitof AR1 models
...
-   We need to assign priors to them
...
If you want to use a continuous model for time (or for smoothing a covariate) you need to:
