...
::: notes
-   "solving the SPDE" means finding a random function $Z(t)$ such that the equality $(\kappa^2-\Delta)^{\alpha/2}Z(t) = W(t)$ holds in distribution.
-   in other words, finding $Z(s)$ such that when the operator $(\kappa^2-\Delta)^{\alpha/2}$ is applied to it, it produces white noise
...
-   The Matérn field is infinite-dimensional so we approximate it using a finite basis expansion
    -   we want a simple local basis
    -   each basis only affects neighbours
    -   so computations become sparse
-   Each weight $w_i$ is basically the value of the field at node $i$
-   The basis functions are:
    -   Height = 1 at node
